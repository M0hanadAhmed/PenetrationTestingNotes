...
**Category:** Web Exploitation → Command Injection → Credential Hunting → Offline Cracking
**Target:** CorpNet's internal Network Operations Centre (NOC)

> "The portal is up. The services show green. The audit log looks clean. But clean logs can be written by anyone."

This room chains four very different skills together: SQL injection to bypass a login form, command injection through a "health check" feature that's filtering shell metacharacters, credential discovery via a leaked config file, and offline password cracking against a KeePass vault. Each stage hands you exactly what the next one needs — a genuinely well-constructed capstone chain.
